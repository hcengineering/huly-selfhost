...
| `praut-import-md-docs.cjs` | Import `.md` příloh z chatu jako nativní Huly Dokumenty (řeší „soubor se stáhne místo zobrazení"). Stáhne blob z front `/files/<wsUuid>?file=<blobId>`, `markdownToMarkup`→`jsonToMarkup`→blob, `createDoc` dokumentu do teamspace (default „Podklady k tierům", vytvoří ho když chybí). Idempotentní (skip dle titulku). DRY-RUN bez `--apply`. **NODE_PATH musí zahrnout i `packages/importer/node_modules`** (kvůli `@hcengineering/text-markdown` + `@hcengineering/rank`). Seznam souborů = jednorázový read-only SQL `public.attachment` (admin je přes API nevidí — není členem kanálu). |
