...
- 2026-07-13 — **Zprovozněn chat (DM + upozornění):** v produkčním compose chyběly služby **`notification`** (web-push; nešla nastartovat bez VAPID klíčů) a **`hulypulse`** (presence, nginx `/_pulse` byl zakomentovaný) — proto se zprávy příjemcům neprojevovaly. Nasazeno: vygenerovány VAPID klíče do `huly_v7.conf`, přidány obě služby, propojeny (transactor `PULSE_URL` + `WEB_PUSH_URL`, front `PULSE_URL` + `PUSH_PUBLIC_KEY`), odkomentován nginx `location /_pulse`. **Ověřeno v provozu** (log transactoru: `chunter:ids:DMNotification`; Štěpán + Simon zprávu vidí). Nové nástroje: `praut-diagnose-messaging.cjs` (read-only diagnostika), `praut-enable-chat-services.sh` (dry-run / `--apply`). Detaily + rollback: `docs/handovers/2026-07-08-chat-diagnostika.md`. Zbývá: e-mailová upozornění na DM jsou stále vypnutá (web-push jede).
